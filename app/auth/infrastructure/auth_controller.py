...
async def refresh(
    ...
    """
        Authenticates the user using a refresh token and generates a new access token and a new refresh token.
        ...
        Returns:
            Response: If the refresh token is valid, the response generated by `generate_token_response` is returned.

        Raises:
            AuthTokenError: If the refresh token is missing or invalid, an `AuthTokenError` is raised with an error message.
        """
...
def generate_tokens(
    ...
    Parameters:
        ...
        Defaults to `Depends(Provide["services.jwt"])`.
...
def generate_token_response(response: Response, access_token: str, refresh_token: str) -> Token:
    """
    Generates an HTTP response with an access token and a refresh token as a cookie.

    Parameters:
        response (Response): A FastAPI `Response` object that is used to generate the HTTP
        response for the token endpoint.
        access_token (str): A string that contains the access token to be returned in the response.
        refresh_token (str): A string that contains the refresh token to be set as a cookie in the response.

    Returns:
        Token: A `Token` object containing the `access_token` and a `token_type` of "bearer".
    """
